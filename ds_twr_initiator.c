...
//#define POLL_TX_TO_RESP_RX_DLY_UUS (500 + CPU_PROCESSING_TIME)
//#define POLL_TX_TO_RESP_RX_DLY_UUS (300 + CPU_PROCESSING_TIME)*2
#define POLL_TX_TO_RESP_RX_DLY_UUS 650
//#define POLL_TX_TO_RESP_RX_DLY_UUS 1400
...
//#define RESP_RX_TO_FINAL_TX_DLY_UUS (500 + CPU_PROCESSING_TIME)*2
#define RESP_RX_TO_FINAL_TX_DLY_UUS 900
//#define RESP_RX_TO_FINAL_TX_DLY_UUS 1800
...
static uint16_t cir_length = 30;

#define LCD_BUFF_LEN (500)
static uint8_t usbVCOMout[LCD_BUFF_LEN * 4];
...
int ds_twr_initiator(void)
{
   ...
   * set by dwt_setrxaftertxdelay() has elapsed. */
    ...
    if (ret == DWT_SUCCESS)
    {
        ...
        final_tx_time = (poll_tx_ts + (10000 * UUS_TO_DWT_TIME)) >> 8;
...
//            uint8_t current_idx = rx_buffer[ALL_MSG_SN_IDX];
...
//                    uint8_t cir_buffer[cir_length*6+1];

//                    dwt_readaccdata(cir_buffer, cir_length*6+1, 730);

//                    struct cir_tap_struct *cir = (struct cir_tap_struct *) &cir_buffer[1];

//                    for (int j = 0; j < cir_length; j++)
//                    {
//                            n += sprintf((char *)&usbVCOMout[n], "%06x,%06x,", cir[j].real, cir[j].imag);
//                    }

//                    n += sprintf((char *)&usbVCOMout[n], "%04x,%04x,%0x,0 \r\n", rx_diag.ipatovFpIndex, rx_diag.ipatovPOA, frame_seq_nb);

//                    //test_run_info((uint8_t *)usbVCOMout);
